"""Finite daily integration for TASK-151 schedule-aware MPC cycles.

This parallel runner consumes the outer TASK-151 cycle once per caller-owned
hour.  Its physical view is a compatibility artifact only: existing
explanation, journal, and CSV contracts receive that exact view without
constructing another optimization or decision path.
"""
...
class MultiOpportunityExplainableMPCDailySimulationInput:
    """Add TASK-151 caller-owned configurations to existing daily facts."""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.daily_mpc_input, ExplainableMPCDailySimulationInput):
            raise TypeError(
                "daily_mpc_input must be an ExplainableMPCDailySimulationInput"
            )
        if not isinstance(
...
@dataclass(frozen=True, slots=True)
class MultiOpportunityExplainableMPCDailySimulationStepTrace:
    """Retain one outer TASK-151 cycle and exact downstream evidence."""

    context: EMSContext
    forecast_horizon: ForecastHorizon
    multi_opportunity_mpc_cycle_result: MultiOpportunityMPCCycleResult
    ...
    def __post_init__(self) -> None:
        ...
        cycle = self.multi_opportunity_mpc_cycle_result
...
@dataclass(frozen=True, slots=True)
class MultiOpportunityExplainableMPCDailySimulationResult:
    """Retain all completed daily evidence and exactly one final CSV write."""

    source_input: MultiOpportunityExplainableMPCDailySimulationInput
    simulation_result: DailySimulationResult
    step_traces: tuple[MultiOpportunityExplainableMPCDailySimulationStepTrace, ...]
...
class MultiOpportunityExplainableMPCDailySimulationBoundary(ABC):
    """Define one finite caller-requested 24-step schedule-aware simulation."""
...
@dataclass(frozen=True, slots=True)
class MultiOpportunityExplainableMPCDailySimulationRunner(
    MultiOpportunityExplainableMPCDailySimulationBoundary
):
    """Compose 24 caller-owned TASK-151 cycles with actual simulator feedback."""

    mpc_cycle_boundary: MultiOpportunityMPCCycleBoundary
    ...
    def run(
        ...
        daily_traces: list[MultiOpportunityExplainableMPCDailySimulationStepTrace] = []
        ...
        return MultiOpportunityExplainableMPCDailySimulationResult(
            source_input,
            simulation_result,
            tuple(daily_traces),
            tuple(records),
    ...
    @staticmethod
    def _require_cycle(result: object, source: MultiOpportunityMPCCycleInput) -> None:
        if not isinstance(result, MultiOpportunityMPCCycleResult):
            raise TypeError(
                "mpc_cycle_boundary must return a MultiOpportunityMPCCycleResult"
            )
    ...
    def _require_feasible(
        feasible: object,
        cycle: MultiOpportunityMPCCycleResult,
        provenance: DecisionProvenance,
    ) -> None:
