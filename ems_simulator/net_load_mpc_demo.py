"""Runnable net-load-aware 24-hour explainable MPC comparison demo.

This module deliberately keeps the TASK-129 price-only demo untouched.  It
reuses the same caller-owned scenario and downstream evidence/execution seams,
but replaces only the candidate optimization boundary used before physical
revision.
"""
...
class NetLoadAwareMPCDemoExecutionResult:
    """Preserve exact completed net-load-aware demo artifacts."""
...
def create_demo_input(output_directory: Path) -> ExplainableMPCDailySimulationInput:
    """Create the TASK-129 scenario with the net-load candidate boundary."""

    if not isinstance(output_directory, Path):
        raise TypeError("output_directory must be a pathlib.Path")
    daily_input = _daily_input()
    capability = CapabilityDescriptor("net-load-mpc", "Physically-aware net-load MPC.")
    ...
    return ExplainableMPCDailySimulationInput(
        ...
        EMSStrategyDescriptor("physically-aware-net-load-mpc", "1.0"),
...
def run_demo(output_directory: Path) -> NetLoadAwareMPCDemoExecutionResult:
    """Run the finite net-load-aware day and write the five comparison files."""
    ...
    summary_path.write_text(
        _summary_text(result, simulation_export.summary), encoding="utf-8"
    )
    return NetLoadAwareMPCDemoExecutionResult(
        source_input,
        result,
        simulation_export,
        simulation_paths,
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run the net-load-aware daily MPC comparison CLI."""

    parser = argparse.ArgumentParser(description="EOS net-load-aware MPC daily demo")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("simulation_output_net_load_mpc"),
        help="directory for deterministic net-load-aware demo outputs",
    )
    arguments = parser.parse_args(argv)
    execution = run_demo(arguments.output_dir)
    print(f"Net-load-aware MPC decisions: {execution.decision_csv_path}")
...
def _runner() -> ExplainableMPCDailySimulationRunner:
    """Compose the existing physical, explanation, and execution seams."""
...
def _daily_input() -> DailySimulationScenarioInput:
    """Create the exact TASK-129 caller-owned daily scenario."""
...
def _forecast_horizons(
    ...
    """Create the same explicit four-point repeating-day perfect forecast."""
...
def _summary_text(
    ...
    """Render the same stable metrics as TASK-129 with a distinct heading."""
    ...
    return (
        "EOS Net-Load-Aware Explainable MPC Daily Demo Summary\n"
