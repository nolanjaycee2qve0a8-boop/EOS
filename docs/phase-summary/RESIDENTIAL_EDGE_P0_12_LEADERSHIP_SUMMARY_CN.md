...
- 本地验证覆盖 focused、Edge Runtime、Residential frozen、Campaign A–F、full pytest、静态检查、pre-commit 与八项 mutation。
...
本文件记录的是本地候选；尚未 push、PR 或合并。下一步应先进行独立只读复审，再由用户决定是否发布。若未来进入产品化，应以真实 telemetry、设备协议、故障模型和 HIL 证据验证，而不是把本审计合同当作设备认证。
