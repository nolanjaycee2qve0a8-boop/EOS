...
之外，是一个同步、无状态的审计层：
`caller transmission + ACK + actual + declared source/epoch relationship -> PASS/GAP`。
assessment 只保留 audit identity、as-of、status 与 findings，不保留 input 或任何 live
...
## 30. P0.8 Adapter Conformance Harness（PR #200 已合并 main）
