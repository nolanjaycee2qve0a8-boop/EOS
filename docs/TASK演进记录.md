...
# Residential Simulation Validation Campaign B — Physical & Economic Boundary Sweep

Campaign B is post-freeze validation tooling, not a new TASK and not a control-capability change. It adds an explicit 72-cell deterministic matrix: B1 PCS boundary sweep (18), B2 initial-SOC sweep (12), B3 tariff-opportunity sweep (18), and B4 explicit accounting-only sensitivity (24). Each cell retains caller-owned perfect-forecast facts and existing TASK-176 physical, reconciliation, provenance, and explainability acceptance.

B1/B2/B3 execute the frozen Schedule-aware and Economic Schedule-aware paths once per scenario. B4 deliberately reuses exact completed source trajectories (three source pairs) and only rebuilds the TASK-173 ledger and TASK-174 comparison under explicit export/degradation/terminal accounting assumptions; it never reruns control. Campaign B transparently reports 72 logical scenarios / 144 path records and 102 unique control executions. Ranking shifts, non-monotonicity and economic losses are human-review observations, never hard acceptance rules. The reporting SVGs place zero axes at their computed baselines and expose their exact swept input plus environment in escaped deterministic labels; B4 labels separately identify export tariff, degradation rate, and terminal value. Its handoff is a future caller-supplied forecast-error campaign while functional freeze remains active.
...
显式 one-shot continuation 串接多个这样的 cycle。caller 从不提供或重建 `PowerCommand`；它只在 P0.6 内由
...
## Edge P0.9 Device-Fact Readiness Profile 学习材料（PR #203 已合并 main）

P0.9 仅新增 test-only、同步、caller-driven 的设备事实 readiness evaluator：以 caller-owned `as_of`、`max_age`、profile 与 evidence 形成 immutable PASS/GAP 审计，不拥有 clock、command、runtime、adapter 或设备 authority。最终本地证据为 focused、Residential frozen `530 passed, 62 deselected`、Campaign A–F `62 passed`、full pytest `2736 passed`、静态门禁、早期代码阶段 pre-commit 与七项有效 mutation；ACK 六字段缺失也 fail closed。学习示例格式修正后的隔离本地 pre-commit 因相对项目 `.pytest_cache` basetemp 的 Windows `WinError 5` 失败（pytest hook `2587 passed, 149 errors`），不计为 PASS；PR #203 的 Quality checks SUCCESS 随后独立验证了格式修正。它已通过 PR #203 普通合并到 main（`4690d47cbfa4cac0ae4eb4e9a27b722d68aa17a7`）。这不表示 hardware/field readiness、物理完成或设备 authority；下一阶段仍须 capability-gap review 与显式用户批准。
