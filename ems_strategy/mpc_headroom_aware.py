"""One headroom-aware MPC cycle with a physical explanation compatibility view."""
...
class HeadroomAwareMPCCycleResult:
    """Retain headroom, physical, and current-decision provenance in one cycle."""
    ...
    headroom_optimization_output: HeadroomAwarePhysicalOptimizationSolveOutput
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(
            self.headroom_optimization_output,
            HeadroomAwarePhysicalOptimizationSolveOutput,
        ):
            raise TypeError(
                "headroom_optimization_output must be a "
                "HeadroomAwarePhysicalOptimizationSolveOutput"
            )
        ...
        output = self.headroom_optimization_output
        physical_output = output.physical_output
        if output.source_input is not self.physically_aware_input:
            raise ValueError("headroom output must preserve exact physical input")
        ...
        if (
            ...
            raise ValueError(
                "physical output must consume exact headroom-aware final candidate"
            )
...
class HeadroomAwareMPCCycleBoundary(ABC):
    """Define one stateless headroom-aware MPC planning cycle only."""
    ...
    def run_cycle(
        ...
        """Run one complete headroom-aware planning-to-decision cycle."""
        raise NotImplementedError


@dataclass(frozen=True, slots=True)
class HeadroomAwareSingleMPCCycleOrchestrator(HeadroomAwareMPCCycleBoundary):
    """Coordinate exactly one TASK-136 output into a current MPC decision."""

    headroom_optimization_boundary: HeadroomAwarePhysicalOptimizationBoundary
    ...
    def __post_init__(self) -> None:
        if not isinstance(
            self.headroom_optimization_boundary,
            HeadroomAwarePhysicalOptimizationBoundary,
        ):
            raise TypeError(
                "headroom_optimization_boundary must be a "
                "HeadroomAwarePhysicalOptimizationBoundary"
            )
        if not isinstance(
    ...
    def run_cycle(
        ...
        output = self.headroom_optimization_boundary.solve_headroom_aware(
            physical_input
        )
    ...
    def _require_output(
        ...
        if not isinstance(output, HeadroomAwarePhysicalOptimizationSolveOutput):
            raise TypeError(
                "headroom_optimization_boundary must return a "
                "HeadroomAwarePhysicalOptimizationSolveOutput"
            )
